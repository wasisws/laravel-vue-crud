            ...
            <nav aria-label="Page navigation">
              <ul class="pagination">
                <li :class="{ disabled: posts.current_page === 1 }">
                  <button @click="fetchData(posts.current_page - 1)" class="page-link" aria-label="Previous">
                    <span aria-hidden="true">&laquo;</span>
                  </button>
                </li>
                <li v-for="page in posts.last_page" :key="page" :class="{ active: page === posts.current_page }">
...
export default {
  setup() {
    ...
    const deleteSiswa = (siswaId) => {
      if (confirm("Apakah Anda yakin ingin menghapus siswa ini?")) {
        axios
          .delete(`http://localhost:8000/api/siswa/${siswaId}`)
          .then(() => {
            fetchData(posts.value.current_page);
          })
